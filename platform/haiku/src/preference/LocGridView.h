...
#ifndef LocGridView_h
#define LocGridView_h
...
public:
    ...
    void SetColumnWidth(size_t n, float width);
    void AddColumn(const char* label, int32 ratio=10);
    const char* GetColumn(size_t n) const;
...
protected:
    ...
    std::vector<int32> mnTitleWidthsRatio;
    float mnHeight;
...
template<typename T>
class CheckBoxColumn : public GridColumn<T>
{
public:
    CheckBoxColumn(BView* view, int32 column);
...
template<typename T>
CheckBoxColumn<T>::CheckBoxColumn(BView* view, int32 column)
    : GridColumn<T>(view, column, NULL)
{
}

template<typename T>
void CheckBoxColumn<T>::DrawAt(BView* view, BRect rect, bool selected, T* pRow)
{
    rgb_color base = ui_color(B_PANEL_BACKGROUND_COLOR);
    uint32 flags = pRow->IsActive() ? BControlLook::B_ACTIVATED : 0;
    BFont font;
    view->GetFont(&font);
    font_height fontHeight;
    view->GetFontHeight(&fontHeight);
    BRect checkBoxRect(0.f, 2.f, ceilf(3.f + fontHeight.ascent),
            ceilf(5.f + fontHeight.ascent));
    checkBoxRect.OffsetBy(rect.LeftTop());
    checkBoxRect.OffsetBy(rect.Size().width / 2 - checkBoxRect.Size().width / 2, 0);
    
    rgb_color highColor = view->HighColor();
    const float penSize = view->PenSize();
    
    be_control_look->DrawCheckBox(view, checkBoxRect, rect, base, flags);
...
template<typename T>
void CheckBoxColumn<T>::Edit(BView* view, BRect rect, BPoint pos, T* pRow)
{
    if (pRow) {
        _FlipState(pRow);
        view->Invalidate();
...
public:
    ...
    void MoveRow(int32 index, bool up);
...
template<typename T>
void GridView<T>::SelectRow(int32 row)
{
    _SelectRow(row);
}
...
template<typename T>
void GridView<T>::_SelectionChanged()
{
}
...
template<typename T>
void GridView<T>::MoveRow(int32 index, bool up)
{
    if (0 <= index && index < mpRows.size()) {
        if (up) {
            if (index == 0) {
            ...
            T* row = mpRows[index];
            mpRows[index] = mpRows[index - 1];
            mpRows[index - 1] = row;
        } else {
            if (index == mpRows.size() - 1 || mpRows.size() - 1 < index + 1) {
                return;
            }
            T* row = mpRows[index];
            mpRows[index] = mpRows[index + 1];
